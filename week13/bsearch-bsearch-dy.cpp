#include "./common.cpp"
#include <stdlib.h>
#include <algorithm>
using namespace std;

unsigned num = 16 * 1024 * 1024; // maximum num of inputs
unsigned bound = 1000 * 1000; // numbers will be ranged in [0..bound)

int compare(const void* lptr, const void* rptr) {
    // REMIND
    // reinterpret_cast() 사용하는 거 유의!
    unsigned l = *(reinterpret_cast<const unsigned*>(lptr));
    unsigned r = *(reinterpret_cast<const unsigned*>(rptr));

    return l - r;
}

int main(const int argc, const char* argv[]) {
    // argv processing
	switch (argc) {
	case 1:
		break;
	case 2:
		num = procArg( argv[0], argv[1], 1024 );
		break;
	case 3:
		num = procArg( argv[0], argv[1], 1024 );
		bound = procArg( argv[0], argv[2], 1024 );
		break;
	default:
		printf("usage: %s [num] [bound]\n", argv[0]);
		exit(EXIT_FAILURE); // EINVAL: invalid argument
		break;
	}
	printf("num = %d, bound = %d\n", num, bound);

    unsigned* vecData = (unsigned*)malloc(num*sizeof(unsigned));

    srand(0);
    setRandomData<unsigned>(vecData, num, bound);
    unsigned targetValue = vecData[num-1];
    printf("target value = %d\n", targetValue);

    std::sort(vecData, vecData + num);

    ELAPSED_TIME_BEGIN(0);
    unsigned* foundPtr = (unsigned*)bsearch(&targetValue, vecData, num, sizeof(unsigned), compare);
    ELAPSED_TIME_END(0);

    if (foundPtr == nullptr) {
        printf("NOT FOUND: target value '%u' not found\n", targetValue);
    } else {
        unsigned index = foundPtr - vecData;
        printf("FOUND: vecData[%d] = %u\n", index, vecData[index]);
    }

    printVec("vecData", vecData, num);
    free(vecData);

    return 0;
}
...
__global__ void kernelMatTransposeOnlySquareMat(float* dst, float* src, int n, size_t pitchInElem) {
    unsigned int gy = blockIdx.y * blockDim.y + threadIdx.y;
    unsigned int gx = blockIdx.x * blockDim.x + threadIdx.x;

    if (gy < n && gx < n) {
        // REMIND: bank comflict를 피하면 성능 상당히 개선됨..! Wow!
        ...
        unsigned int dstIdx = gy * pitchInElem + gx;
    }
}

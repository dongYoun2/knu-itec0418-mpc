...
const unsigned TOTAL_NUM = (2 * BLOCK_SIZE) * 16;
...
int main(const int argc, const char* argv[]) {
    ...
    if (direction == INCREASING) {
        for (int i = 0; i < numUnits; i++) {
            std::sort(dst + i * unitSize, dst + (i + 1) * unitSize);
        }
    } else {
        for (int i = 0; i < numUnits; i++) {
            std::sort(dst + i * unitSize, dst + (i + 1) * unitSize, std::greater<uint32_t>());
        }
    }
    ELAPSED_TIME_END(0);

    // getTotalDiff() 에 사용하기 위해
    if (direction == INCREASING) {
		for (unsigned i = 0; i < numUnits; ++i) {
			qsort( src + i * (2 * BLOCK_SIZE), (2 * BLOCK_SIZE), sizeof(uint32_t), compareLess );
		}
	} else {
		for (unsigned i = 0; i < numUnits; ++i) {
			qsort( src + i * (2 * BLOCK_SIZE), (2 * BLOCK_SIZE), sizeof(uint32_t), compareGreater );
		}
	}
